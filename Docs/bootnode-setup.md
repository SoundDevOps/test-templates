...
ssh-keygen -t rsa
```
![SSH-term](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/gen_ssh_term.png)
...
![PuTTY](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/Putty.png)
...
On this step you will create azure virtual machine from a template by filling in a number of fields with data obtained on previous steps. After virtual machine deployment is complete, it will automatically start new Musereum-PoA network.
...
[![Deploy to Azure](http://azuredeploy.net/deploybutton.png)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fmusereum%2Ftest-templates%2Fdev%2FTestTestNet%2Fbootnode%2Ftemplate.json)
...
4. **Resource group**: Choose "Create new" Resource group and input a name for the resource group. This name will be displayed on your azure dashboard, it will not be used in the Musereum-PoA network, so choose a name that would make it clear _to you_ what this resource group represents. However, Azure imposes certain restrictions on a resource group name: it can only include upper case and lower case latin letters, numbers (e.g. _a_, _B_, _5_), periods, underscores, hyphens and parenthesis, cannot contain spaces (` `) and cannot end in a period. An example of a correct name is `musereum-poa`. After you've typed in the name, make sure a green check mark ✓ appears on the right.
5. **Location**: Select a location to where the virtual machine will be deployed.
6. **Node Full Name**: Enter full name of the bootnode. This will be displayed in the Musereum-PoA network usage info page and visible to other users of the network.
...
![wizard-1](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/deployment1.png)
...
![wizard-2](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/deployment2.png)

15. Click "Purchase". In case of errors please double check that you have completed the steps above and all fields are filled with correct values. If the error persists you can file a bug report [here](https://github.com/musereum/test-templates/issues/new). Please provide as detailed a description as possible, one or several screenshots, so that values in all fields will be visible to us. Also provide a screenshot with the error message.

16. After that, you will be taken to your azure dashboard. Look for a box similar to this
![Deployment in progress](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/deploy_new_deployment.png)
...
![resources](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/deployed-resources.png)
...
![ls_home](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/ls_home.png)
...
![ls_logs](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/ls_logs.png)
...
![pm2_list_azureuser](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/pm2_list_azureuser.png)
![pm2_list_root](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/pm2_list_root.png)
...
![grep_enode](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/grep_enode.png)

copy enode url address, its format is `enode://<128 hex characters>@ip_address:30300`.
In repository, open file [`TestTestNet/mining-node/node.toml`](https://github.com/musereum/test-templates/blob/dev/TestTestNet/mining-node/node.toml) and replace the value in `bootnodes` parameter with the url above like so:
```
...
...
Connect to the virtual machine, `cd musereum-initial-keys` and run the following command to generate initial keys
```
...
![initial_keys_generated](https://raw.githubusercontent.com/musereum/test-templates/dev/Docs/initial_keys_generated.png)
